...
export function Telephone(props: { callIntent: AppCallIntent; backToContacts: () => void }) {
  // state
  ...
  const { chatTitle, reMessages } = useChatStore(
    useShallow((state) => {
  ...
  React.useEffect(() => {
    ...
    const chatMessages: { role: VChatMessageIn['role']; text: string }[] =
      reMessages && reMessages.length > 0 ? reMessages : personaSystemMessage ? [{ role: 'system', text: personaSystemMessage }] : [];
      ...
      .finally(() => {
        ...
        if (finalText || error)
          setCallMessages((messages) => [
            ...messages,
            createDMessage('assistant', finalText + (error ? ` (ERROR: ${error.message || error.toString()})` : '')),
          ]);
        // fire/forget
        if (finalText?.length >= 1) void EXPERIMENTAL_speakTextStream(finalText, personaVoiceId);
      });
        ...
        >
          <ScrollToBottom stickToBottomInitial>
            <Box sx={{ minHeight: '100%', p: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
              ...
              {!!personaTextInterim && <CallMessage text={personaTextInterim} variant="outlined" color="neutral" role="assistant" />}
              ...
              {isRecording && (
                <CallMessage
                  text={
                    <>
                      {speechInterim?.transcript.trim() || null}
                      {speechInterim?.interimTranscript.trim() ? <i> {speechInterim.interimTranscript}</i> : null}
                    </>
                  }
